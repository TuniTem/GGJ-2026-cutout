...
func _init() -> void:
	...
	Steam.steamInit(false, APP_ID)
	
func _ready() -> void:
	var initialize_response : String = Steam.steamInitEx(APP_ID, true)["verbal"]
	Debug.push("Steam initalization : " + ("Successful" if initialize_response == "" else initialize_response), Debug.INFO)
